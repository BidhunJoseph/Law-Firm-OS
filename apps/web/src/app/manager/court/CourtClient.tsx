...
import { createCourtEvent } from "@/server/actions/court-actions";
...
const EVENT_TYPES = [
  ...
  { id: "Adjournment", label: "Adjournment", color: "bg-orange-100 text-orange-800 border-orange-200" },
        ...
        <Select.Viewport className="p-1">
          {options.map((opt) => (
            <Select.Item
              key={opt.id}
              value={opt.id}
              className="relative flex cursor-pointer select-none items-center rounded-sm py-1.5 pl-8 pr-2 text-sm text-gray-900 outline-none transition-colors hover:bg-gray-100 focus:bg-gray-100 data-[disabled]:pointer-events-none data-[disabled]:opacity-50"
            >
...
export default function CourtClient({ initialEvents, cases }: { initialEvents: any[]; cases: any[] }) {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.refresh(); // Fetch new events
    } catch (err) {
      console.error(err);
        ...
        <div className="flex w-full items-center gap-2 sm:w-auto">
          <div className="relative flex-1 sm:w-64">
            ...
            <input
              ...
              className="h-8 w-full text-gray-900 rounded-md border border-gray-300 bg-white pl-8 pr-3 text-sm shadow-sm transition-colors focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
            />
          </div>
              ...
              <Dialog.Content className="fixed left-[50%] top-[50%] z-50 w-[95vw] max-w-lg translate-x-[-50%] translate-y-[-50%] rounded-xl bg-white shadow-xl duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%]">
                ...
                <form onSubmit={handleFormSubmit} className="p-5 overflow-y-auto max-h-[80vh]">
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <CustomSelect 
                        ...
                        options={cases.map(c => ({id: c.id, label: `${c.id.slice(0, 8)} - ${c.title}`}))}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input 
                          ...
                          className="h-8 w-full text-gray-900 rounded-md border border-gray-300 px-3 text-sm shadow-sm transition-colors focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                        />
                      </div>
                      <div>
                    ...
                    <div>
                      ...
                      <textarea 
                        ...
                        className="w-full text-gray-900 rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm transition-colors focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 resize-none"
                      />
                    </div>
                  </div>
                  ...
                  <div className="mt-6 flex justify-end gap-2 pt-4 border-t border-gray-100">
                    ...
                    <button type="submit" disabled={isSubmitting} className="rounded-md bg-blue-600 px-4 py-1.5 text-sm font-medium text-white shadow-sm transition-colors hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 disabled:opacity-50">
                      Save Event
                    </button>
                  </div>
                </form>
          ...
          <div className="flex flex-1 flex-col overflow-hidden bg-white">
            ...
            <div className="flex-1 grid grid-cols-7 grid-rows-5 overflow-y-auto">
              {calendarDays.map((day, idx) => {
                const dayEvents = initialEvents.filter(e => isSameDay(new Date(e.event_date), day));
                ...
                return (
                  ...
                  >
                    ...
                    <div className="space-y-1.5 px-0.5 overflow-y-auto max-h-[calc(100%-28px)] no-scrollbar">
                      {dayEvents.map(event => {
                        const eventType = EVENT_TYPES.find(t => t.id === event.event_type) || EVENT_TYPES[0];
                        ...
                        return (
                          <div 
                            ...
                            title={`${event.title} - ${meta.court || ''}`}
                          >
                            <div className="flex items-center gap-1 truncate">
                              <span className="font-bold opacity-80">{format(new Date(event.event_date), "h:mm a")}</span>
        ...
        {view === "list" && (
          <div className="flex-1 overflow-auto bg-gray-50/30 p-4 sm:p-6">
            <div className="max-w-4xl mx-auto space-y-3">
              {initialEvents.sort((a,b) => new Date(a.event_date).getTime() - new Date(b.event_date).getTime()).map(event => {
                const eventType = EVENT_TYPES.find(t => t.id === event.event_type) || EVENT_TYPES[0];
                const caseObj = cases.find(c => c.id === event.case_id);
                const meta = event.metadata as any || {};
                const statusObj = STATUS_OPTIONS.find(s => s.id === meta.status);
                ...
                return (
                  <div key={event.id} className="group flex flex-col sm:flex-row items-start sm:items-center gap-4 rounded-xl border border-gray-200 bg-white p-3 shadow-sm transition-all hover:shadow-md hover:border-gray-300">
                    <div className="flex flex-col items-center justify-center min-w-[72px] rounded-lg bg-gray-50 p-2 border border-gray-100">
                      <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">{format(new Date(event.event_date), "MMM")}</span>
                      <span className="text-2xl font-bold text-gray-900 leading-none mt-0.5">{format(new Date(event.event_date), "dd")}</span>
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500 mt-0.5 truncate">{caseObj?.title || 'Unknown Case'} <span className="text-gray-400 ml-1">({caseObj?.id?.slice(0, 8)})</span></p>
                    </div>
                    ...
                    <div className="flex-1 min-w-0 text-xs text-gray-600 space-y-1.5 sm:pl-4 sm:border-l sm:border-gray-100">
                      ...
                      <div className="flex items-center gap-2 truncate">
                        ...
                        {meta.court || "No court specified"}
                      </div>
                      <div className="flex items-center gap-2 truncate">
                        <Gavel className="h-3.5 w-3.5 text-gray-400 shrink-0" />
                        {meta.judge || "No judge specified"}
                      </div>
                    </div>
